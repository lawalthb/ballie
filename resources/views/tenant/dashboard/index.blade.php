...
<div class="space-y-8">
        ...
        <div class="relative z-10">
            <div class="flex items-center justify-between">
                ...
                <div class="hidden md:flex space-x-4">
                    <a href="{{ route('tenant.invoices.create', ['tenant' => tenant()->slug]) }}" class="bg-white bg-opacity-20 hover:bg-opacity-30 text-white px-6 py-3 rounded-xl font-semibold transition-all duration-300 backdrop-blur-sm border border-white border-opacity-20">
                        Create Invoice
                    </a>
        ...
        <div class="metric-card rounded-2xl p-6 shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold gradient-text">₦{{ number_format($totalRevenue, 0) }}</p>
        ...
        <div class="metric-card rounded-2xl p-6 shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold gradient-text">₦{{  5000}}</p>
                    <div class="flex items-center mt-2">
        ...
        <div class="metric-card rounded-2xl p-6 shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold gradient-text">₦{{  5001}}</p>
                    <div class="flex items-center mt-2">
                        <span class="text-green-500 text-sm font-medium flex items-center">
                            <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M5.293 9.707a1 1 0 010-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414L11 7.414V15a1 1 0 11-2 0V7.414L6.707 9.707a1 1 0 01-1.414 0z" clip-rule="evenodd"></path>
        ...
        <div class="metric-card rounded-2xl p-6 shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold gradient-text">₦{{ number_format(55, 0) }}</p>
                    <div class="flex items-center mt-2">
    ...
    <div class="bg-white rounded-2xl p-6 shadow-lg">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            <a href="{{ route('tenant.invoices.create', ['tenant' => tenant()->slug]) }}" class="quick-action-btn bg-gradient-to-br from-blue-500 to-blue-600 text-white p-4 rounded-xl text-center shadow-lg">
            ...
            <a href="{{ route('tenant.customers.create', ['tenant' => tenant()->slug]) }}" class="quick-action-btn bg-gradient-to-br from-green-500 to-green-600 text-white p-4 rounded-xl text-center shadow-lg">
                <div class="w-8 h-8 mx-auto mb-2">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('tenant.products.create', ['tenant' => tenant()->slug]) }}" class="quick-action-btn bg-gradient-to-br from-purple-500 to-purple-600 text-white p-4 rounded-xl text-center shadow-lg">
                <div class="w-8 h-8 mx-auto mb-2">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-lg">
                ...
                <div class="space-y-4">
                    @foreach($alerts as $alert)
                        <div class="flex items-start space-x-3 p-3 bg-{{ $alert['color'] }}-50 rounded-xl border-l-4 border-{{ $alert['color'] }}-500">
                            <div class="w-8 h-8 bg-{{ $alert['color'] }}-500 rounded-full flex items-center justify-center flex-shrink-0">
                            ...
                            <div class="flex-1">
                                <p class="text-sm font-medium text-{{ $alert['color'] }}-800">{{ $alert['title'] }}</p>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-lg">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm font-semibold text-gray-900">₦{{ number_format($quickStats['monthly_sales']) }}</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div class="bg-gradient-to-r from-blue-500 to-blue-600 h-2 rounded-full" style="width: {{ $quickStats['monthly_sales_percentage'] }}%"></div>
                    </div>

                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600">Customer Growth</span>
                        <span class="text-sm font-semibold text-green-600">+{{ $quickStats['customer_growth'] }}%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div class="bg-gradient-to-r from-green-500 to-green-600 h-2 rounded-full" style="width: {{ $quickStats['customer_growth'] }}%"></div>
                    </div>

                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600">Expense Ratio</span>
                        <span class="text-sm font-semibold text-yellow-600">{{ $quickStats['expense_ratio'] }}%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 h-2 rounded-full" style="width: {{ $quickStats['expense_ratio'] }}%"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                @foreach($recentTransactions as $transaction)
                    <div class="transaction-row flex items-center justify-between p-4 rounded-xl">
                        <div class="flex items-center space-x-4">
                            <div class="w-10 h-10 bg-gradient-to-br from-{{ $transaction['icon_color'] }}-400 to-{{ $transaction['icon_color'] }}-600 rounded-full flex items-center justify-center">
                ...
                @foreach($recentActivities as $activity)
                    <div class="activity-item">
                        <div class="flex items-start space-x-3">
                            <div class="w-8 h-8 bg-{{ $activity['icon_color'] }}-100 rounded-full flex items-center justify-center flex-shrink-0">
                                @if($activity['type'] === 'customer_added')
                                ...
                                @elseif($activity['type'] === 'low_stock')
                                    <svg class="w-4 h-4 text-{{ $activity['icon_color'] }}-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path>
                                    </svg>
                                @else
     ...
     <div class="bg-white rounded-2xl p-6 shadow-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach($upcomingDueDates as $dueDate)
                <div class="bg-gradient-to-br from-{{ $dueDate['color'] }}-50 to-{{ $dueDate['color'] }}-100 p-4 rounded-xl border-l-4 border-{{ $dueDate['color'] }}-500">
                    <div class="flex items-center justify-between mb-2">
                        <span class="status-badge bg-{{ $dueDate['color'] }}-500 text-white text-xs font-medium px-2 py-1 rounded-full">{{ $dueDate['status'] }}</span>
                        <span class="text-sm text-{{ $dueDate['color'] }}-600 font-medium">
                            @if($dueDate['type'] === 'overdue')
                                {{ $dueDate['due_date']->diffForHumans() }}
                            @else
                                {{ $dueDate['due_date']->diffForHumans() }}
                            @endif
                        </span>
                    </div>
                    <h4 class="font-semibold text-gray-900 mb-1">{{ $dueDate['title'] }}</h4>
                    <p class="text-sm text-gray-600 mb-2">{{ $dueDate['client'] }}</p>
                    <p class="text-lg font-bold text-{{ $dueDate['color'] }}-600">
                        @if($dueDate['amount'])
                            ₦{{ number_format($dueDate['amount']) }}
                        @else
                            Tax Return
                        @endif
                    </p>
